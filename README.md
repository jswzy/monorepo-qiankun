...
└── packages/
    ...
    └── app-ship/             # Vue 3 + ECharts 微应用（航运航线监控）
```
...
每艘船的航次被拆成按时间先后排列的挂靠点，图表与图例共用同一份元数据
（`packages/app-ship/src/data/ships.ts` 的 `ROUTE_POINT_META`）：

| 类型 | 含义 | 颜色 |
| --- | --- | --- |
| 起始装货点 | 首装港，主要货源装船 | 绿 |
| 二次装货点 | 补装港，追加货源 | 浅绿 |
| 临时停靠点 | 锚地待泊 / 运河待闸 / 临时避风 | 橙 |
| 补给港口 | 燃油、淡水、物料补给 | 紫 |
| 卸货点 | 中途卸货作业港 | 红 |
| 终点港口 | 本航次终点港 | 蓝 |
...
船名 / 航次 / 状态 / 航线 / 货量 / 当前船位均取自 `航运物流控制塔_Demo/js/data.js`，并补齐了每艘船的挂靠点序列（ETA/ETD、停留时长、作业量、备注）。船只「照片」用 SVG 绘制的汽车船（船体色相按船区分），演示数据不含真实船照。

### 体验路径

```bash
pnpm dev            # 基座 8000 + 四个微应用并行
